...
//
//  https://github.com/kolyvan/PLmovie
//  this file is part of PLMovie
//  PLMovie is licenced under the LGPL v3, see lgpl-3.0.txt
...
////////////////////////////////////////////////////////////////////////////////
NSString * PLPlayerKitErrorDomain = @"com.qgenius.plplayerkit";
static void FFLog(void* context, int level, const char* format, va_list args);

static NSError * PLmovieError (NSInteger code, id info)
{
...
@interface PLMovieFrame()
@property (readwrite, nonatomic) CGFloat position;
@property (readwrite, nonatomic) CGFloat duration;
@end

@implementation PLMovieFrame
@end

@interface PLAudioFrame()
@property (readwrite, nonatomic, strong) NSData *samples;
@end

@implementation PLAudioFrame
- (PLMovieFrameType) type { return PLMovieFrameTypeAudio; }
@end

@interface PLVideoFrame()
@property (readwrite, nonatomic) NSUInteger width;
@property (readwrite, nonatomic) NSUInteger height;
@end

@implementation PLVideoFrame
- (PLMovieFrameType) type { return PLMovieFrameTypeVideo; }
@end

@interface PLVideoFrameRGB ()
@property (readwrite, nonatomic) NSUInteger linesize;
@property (readwrite, nonatomic, strong) NSData *rgb;
@end

@implementation PLVideoFrameRGB

- (PLVideoFrameFormat) format { return PLVideoFrameFormatRGB; }
- (UIImage *) asImage
{
    UIImage *image = nil;
...
@implementation PLVideoFrameYUV
- (PLVideoFrameFormat) format { return PLVideoFrameFormatYUV; }
@end

@interface PLArtworkFrame()
@property (readwrite, nonatomic, strong) NSData *picture;
@end

@implementation PLArtworkFrame
- (PLMovieFrameType) type { return PLMovieFrameTypeArtwork; }
- (UIImage *) asImage
{
    UIImage *image = nil;
...
@interface PLSubtitleFrame()
@property (readwrite, nonatomic, strong) NSString *text;
@end

@implementation PLSubtitleFrame
- (PLMovieFrameType) type { return PLMovieFrameTypeSubtitle; }
@end

////////////////////////////////////////////////////////////////////////////////

@interface PLMovieDecoder () {
    ...
    NSDictionary        *_info;
    PLVideoFrameFormat  _videoFrameFormat;
    NSUInteger          _artworkStream;
    NSInteger           _subtitleASSEvents;
}
@end

@implementation PLMovieDecoder
...
- (void) setSelectedAudioStream:(NSInteger)selectedAudioStream
{
    ...
    PLMovieError errCode = [self openAudioStream: audioStream];
    if (PLMovieErrorNone != errCode) {
...
- (void) setSelectedSubtitleStream:(NSInteger)selected
{
    ...
    if (selected == -1) {
        ...
        PLMovieError errCode = [self openSubtitleStream:subtitleStream];
        if (PLMovieErrorNone != errCode) {
...
{
    PLMovieDecoder *mp = [[PLMovieDecoder alloc] init];
    if (mp) {
            ...
            error: (NSError **) perror
{
    ...
    PLMovieError errCode = [self openInput: path];
    
    if (errCode == PLMovieErrorNone) {
        
        PLMovieError videoErr = [self openVideoStream];
        PLMovieError audioErr = [self openAudioStream];
        
        _subtitleStream = -1;
        
        if (videoErr != PLMovieErrorNone &&
            audioErr != PLMovieErrorNone) {
    ...
    if (errCode != PLMovieErrorNone) {
        ...
        if (perror)
            *perror = PLmovieError(errCode, errMsg);
        return NO;
    }
...
- (PLMovieError) openInput: (NSString *) path
{
    ...
    if (avformat_find_stream_info(formatCtx, NULL) < 0) {
        ...
        return PLMovieErrorStreamInfoNotFound;
    }

    av_dump_format(formatCtx, 0, [path.lastPathComponent cStringUsingEncoding: NSUTF8StringEncoding], false);
    
    _formatCtx = formatCtx;
    return PLMovieErrorNone;
}

- (PLMovieError) openVideoStream
{
    PLMovieError errCode = PLMovieErrorStreamNotFound;
    _videoStream = -1;
    _artworkStream = -1;
...
- (PLMovieError) openVideoStream: (NSInteger) videoStream
{    
    ...
    if (!codec)
        return PLMovieErrorCodecNotFound;
    ...
    if (avcodec_open2(codecCtx, codec, NULL) < 0)
        return PLMovieErrorOpenCodec;
        
    _videoFrame = av_frame_alloc();

    if (!_videoFrame) {
        avcodec_close(codecCtx);
        return PLMovieErrorAllocateFrame;
    }
...
- (PLMovieError) openAudioStream: (NSInteger) audioStream
{   
...
- (PLMovieError) openSubtitleStream: (NSInteger) subtitleStream
{
    ...
    if (codecCtx->subtitle_header_size) {
        ...
        if (s.length) {
            ...
            NSArray *fields = [PLMovieSubtitleASSParser parseEvents:s];
...
- (PLVideoFrame *) handleVideoFrame
{
    ...
    if (_videoFrameFormat == PLVideoFrameFormatYUV) {
            
        PLVideoFrameYUV * yuvFrame = [[PLVideoFrameYUV alloc] init];
        ...
        PLVideoFrameRGB *rgbFrame = [[PLVideoFrameRGB alloc] init];
...
- (PLAudioFrame *) handleAudioFrame
{
    ...
    PLAudioFrame *frame = [[PLAudioFrame alloc] init];
...
- (PLSubtitleFrame *) handleSubtitle: (AVSubtitle *)pSubtitle
{
    NSMutableString *ms = [NSMutableString string];
        ...
        if (rect) {
            ...
            if (rect->text) { // rect->type == SUBTITLE_TEXT
                ...
                if (s.length) {
                    ...
                    NSArray *fields = [PLMovieSubtitleASSParser parseDialogue:s numFields:_subtitleASSEvents];
                    if (fields.count && [fields.lastObject length]) {
                        
                        s = [PLMovieSubtitleASSParser removeCommandsFromEventText: fields.lastObject];
                        if (s.length) [ms appendString:s];
                    }                    
                }
            }
    ...
    PLSubtitleFrame *frame = [[PLSubtitleFrame alloc] init];
    frame.text = [ms copy];   
...
{
    if (format == PLVideoFrameFormatYUV &&
        _videoCodecCtx &&
        (_videoCodecCtx->pix_fmt == AV_PIX_FMT_YUV420P || _videoCodecCtx->pix_fmt == AV_PIX_FMT_YUVJ420P)) {
        
        _videoFrameFormat = PLVideoFrameFormatYUV;
        return YES;
    }
    
    _videoFrameFormat = PLVideoFrameFormatRGB;
        ...
        if (packet.stream_index ==_videoStream) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    ...
                    PLVideoFrame *frame = [self handleVideoFrame];
                    if (frame) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    ...
                    PLAudioFrame * frame = [self handleAudioFrame];
                    if (frame) {
            ...
            if (packet.size) {
                ...
                PLArtworkFrame *frame = [[PLArtworkFrame alloc] init];
            ...
            while (pktSize > 0) {
                ...
                if (gotsubtitle) {
                    ...
                    PLSubtitleFrame *frame = [self handleSubtitle: &subtitle];
                    if (frame) {
                        [result addObject:frame];
                    }
...
static int interrupt_callback(void *ctx)
{
    if (!ctx)
        return 0;
    __unsafe_unretained PLMovieDecoder *p = (__bridge PLMovieDecoder *)ctx;
...
@implementation PLMovieSubtitleASSParser
